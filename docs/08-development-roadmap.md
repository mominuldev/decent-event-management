...
**Release**
- [~] Zero-downtime deploy pipeline with a tested rollback — `.github/workflows/deploy.yml` builds, gates on `migrate --pretend`, boot-checks, and publishes a tagged image to GHCR on every push to `main`, then deploys it: staging automatically, production on a manual dispatch behind the environment's required reviewers. Both jobs are a deliberate no-op until `STAGING_HOST`/`PRODUCTION_HOST` are set, because no host is chosen yet. **Rollback is still untested and unautomated** — the sha-tagged images make it a `docker compose pull` of the previous tag, but nothing rehearses it and nothing reverts a migration
