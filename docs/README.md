...
All monetary columns are `BIGINT UNSIGNED` holding **paisa** (1 BDT = 100 paisa), with an explicit `currency CHAR(3)` defaulting to `BDT`. Floating-point money causes reconciliation drift against gateway settlement reports. The gateway in scope (PayStation, which aggregates bKash, Nagad, Rocket, Upay and cards) settles in BDT with two decimal places.
