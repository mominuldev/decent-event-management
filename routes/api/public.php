...
use App\Http\Controllers\Api\Public\PayStationReturnController;
...
// Browser return leg for PayStation. Read-only — never trusted to
// transition a payment; the IPN at routes/webhooks.php is the only signal
// that does that (docs/06 §6.6). PayStation accepts a single callback_url
// for every outcome, so unlike the removed SSLCommerz legs there is no
// status segment and no `next` parameter: the payment ULID is in the path
// and the redirect origin comes from config alone. Both verbs, because
// the gateway's return method is not documented and a wrong guess would
// strand the payer on a 405.
Route::match(['get', 'post'], 'payments/paystation/return/{payment:ulid}', PayStationReturnController::class)
    ->name('payments.paystation.return');
