...
class BkashWebhookController extends Controller
{
    #[OAT\Post(
        path: '/webhooks/bkash',
        summary: 'bKash payment gateway IPN callback',
        description: 'Server-to-server instant payment notification (IPN) from bKash, dispatched to '
            .'`ProcessGatewayWebhook::handle(\'bkash\', $request)`. The payload is parsed and signature-verified '
            .'by the resolved gateway adapter\'s `parseWebhook()` (see `PaymentGatewayResolver::forMethod()` — as '
            .'of Phase 2 every gateway name, including `bkash`, resolves to the `FakeGateway` stand-in; a real '
            .'`BkashClient` adapter with bKash-specific token/signature handling lands in Phase 4, per '
