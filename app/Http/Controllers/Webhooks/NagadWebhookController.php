...
class NagadWebhookController extends Controller
{
    #[OAT\Post(
        path: '/webhooks/nagad',
        summary: 'Nagad payment gateway IPN callback',
        description: 'Server-to-server instant payment notification (IPN) from Nagad, dispatched to '
            .'`ProcessGatewayWebhook::handle(\'nagad\', $request)`. The payload is parsed and signature-verified '
            .'by the resolved gateway adapter\'s `parseWebhook()` (see `PaymentGatewayResolver::forMethod()` — as '
            .'of Phase 2 every gateway name, including `nagad`, resolves to the `FakeGateway` stand-in; a real '
            .'`NagadClient` adapter with Nagad-specific RSA-encrypted payload handling lands in Phase 4, per '
