...
- **D7** `payment_method` reaches the DB unvalidated (defaults to `bkash`); no validation of ~~`max_admits`~~ (superseded 2026-09-15: the party is bounded by `PartySize::limitFor()` — the `registration.max_family_size` setting when the type's `allows_family` switch is on, 1 when off; `max_admits` itself is deliberately not a bound — and `TicketTypeResource` publishes the effective figure as `max_party_size`), `allowed_participant_types`, `is_active`/`is_public`, or the sale window.
...
Phase 4A landed the expiry sweeper, the reconciliation job, and a real gateway client — `PayStationClient` since 2026-09-10, `SslCommerzClient` before that. `bkash`/`nagad`/`rocket` still resolve to `FakeGateway`, but they are no longer blocking anything: PayStation's hosted checkout already offers all three, so a direct integration is now an optimisation rather than a prerequisite (Phase 4B).
...
- Real `MailDriver` (Phase 5), real `PayStationClient` (2026-09-10, replacing `SslCommerzClient`) and real `SmsDriver`/`ReveSmsClient` (2026-08-22, REVE Systems — falls back to `FakeSmsDriver` when no credentials are configured), plus fake drivers for what is still vendor-blocked: `FakeGateway` (still resolved for `bkash`/`nagad`/`rocket`) and `FakeWhatsAppDriver`
...
- **The media upload endpoint exists** (`POST /admin/content/media`) — magic-byte type detection, full GD re-encode (strips EXIF/GPS and anything smuggled into the container), randomised filename, JPEG/PNG/WebP — and, since 2026-09-16, SVG. An SVG has no pixels to re-encode, so `SvgSanitizer` does the document equivalent: it parses with the network and entities off and rebuilds a *new* file from an allowlist of shapes, paths, text, gradients, clip paths and presentation attributes — scripts, event handlers, `<foreignObject>`, off-document `href`s and CSS `url()`/`@import` are never copied. `ContentMediaUploadTest` pins it with a hostile logo. Added so the site logo (`site_logo` block on the unrouted `site` page) can be the real vector file. This is the CMS half of D9 only — see D9 above for what it does *not* cover.
...
### 🗄️ Phase 4A (SSLCommerz Sandbox) — buildable-now slice closed 2026-08-04 — **SUPERSEDED 2026-09-10**

> **SSLCommerz was removed on 2026-09-10.** `SslCommerzClient`, `SslCommerzReturnController`, `SslCommerzWebhookController` and their tests are deleted, and `sslcommerz` is no longer a resolvable payment method. This section is kept because the surrounding work it describes — the expiry sweeper (D5), `ReconcilePayments`, and the `RefundPayment` gateway wiring — all survives unchanged, and because the two defects its live-sandbox verification found are the reason PayStation was verified against a live sandbox before it shipped. For what runs today see [§PayStation replaces SSLCommerz](#-paystation-replaces-sslcommerz--2026-09-10).
...
- **`REVESMS_DLR_IP_ALLOWLIST` is an intentional no-op until someone supplies REVE's real source ranges**, same as `PAYSTATION_IPN_IP_ALLOWLIST` — a guessed range silently drops every real receipt, which is worse than leaving the key-pair check as the only gate.
...
A completed gateway payment (SSLCommerz, since replaced by PayStation) left the public return page spinning on **পেমেন্ট নিশ্চিত করা হচ্ছে** for ever, with the money taken. Reported from production; reproduced, fixed and verified against a real sandbox transaction the same day.
...
### 🚨 A stale `bootstrap/cache/config.php` silently pointed the test suite at the dev database — 2026-09-10

Found while running the suite during the PayStation work, and worth knowing because every symptom points somewhere else. At HEAD the suite reported **461 of 805 tests failing** — 328 errors, almost all of them `There is no permission named 'registration.view_any' for guard 'web-admin'`. `config/rbac.php` was intact and `RbacSeeder` was correct, so the catalogue looked fine and the failures looked like an RBAC regression.

**The cause was a cached config file.** `php artisan config:cache` had been run at some point, leaving `bootstrap/cache/config.php` on disk. A cached config **completely ignores environment variables**, including the ones `phpunit.xml` sets, so:

- `DB_DATABASE=decent_event_testing` was ignored and **the tests ran against `decent_event`, the development database** — migrating and truncating it on every run. (That is why the `payments` table was empty at the start of this session.) The stray `decent_event_test_1…8` databases on this machine are leftovers of parallel runs that did the same thing.
- `CACHE_STORE=array` was ignored, so Spatie's permission cache used the real persistent store and **survived between test processes**. `Permission::findOrCreate()` found each name in that stale cache and created no row; `syncPermissions()` then did a real lookup, found nothing, and threw. Hence a correct catalogue, a correct seeder, an empty `permissions` table, and a misleading error.

`php artisan config:clear` took the suite from 344 passing to **810 passing, 0 failing, 2 skipped**. Nothing in the codebase changed.

**What to do about it:** never leave a cached config in a development checkout — `composer dev` does not create one, but a deploy script or a stray `config:cache` will, and it is not obvious afterwards. If a large, incoherent block of tests starts failing — especially on permissions, cache or database identity — run `php artisan config:clear` **before** reading any of the failures. `optimize:clear` also does it.

**Two stale `Http::fake()` patterns were fixed in passing**, and they had been hidden behind the same breakage: `ReveSmsClientTest` sets `services.revesms.base_url` to `smpp.ajuratech.com:7790` but two of its tests still faked `smpp.revesms.com:7790` (missed when commit `030ba89` moved the host). `Http::preventStrayRequests()` caught them the moment the suite could run — which is exactly the defect that test's own docblock warns about, in the file that warns about it.

### 💳 Payments — PayStation

Development *and* production run against **PayStation** (<https://www.paystation.com.bd/documentation>). It is an **aggregator**: one hosted checkout fronts bKash, Nagad, Rocket, Upay, Tap, cards, internet banking and EMI, which is why replacing SSLCommerz with it also removed three separate merchant applications from the critical path. **`paystation` is the public checkout's default gateway** (`services.payment.default_method`, `PAYMENT_DEFAULT_METHOD`); `bkash`/`nagad`/`rocket` remain as method names resolving to `FakeGateway`, so never make one of them the default.
...
Sandbox credentials are **published in PayStation's own documentation** — `104-1653730183` / `gamecoderstorepass` — and are the defaults in `config/services.php` and `.env.example`. They are shared with everyone testing against the sandbox, so treat anything they touch as public.

| Purpose | Endpoint | Credentials |
|---|---|---|
| Create a checkout session | `POST /initiate-payment` | `merchantId` + `password` in the **body** |
| Transaction status (v1) | `POST /transaction-status`, keyed by `invoice_number` | `merchantId` in the **header** |
| Transaction status (v2) | `POST /v2/transaction-status`, keyed by `trxId` | `merchantId` in the **header** |
| IPN | PayStation → your URL, flat JSON | **none** |

`merchant_id`/`password` belong in `config/services.php` + `.env`, never committed and never in an unencrypted `event_settings` row. The mandatory order for every transaction is unchanged and is what keeps the money path honest: **an IPN or a browser return is only ever a prompt → server-to-server `/transaction-status` lookup → amount re-check against `amount_due_paisa` → only then `succeeded`.** PayStation transacts in decimal BDT; that conversion lives inside `PayStationClient` alone and no decimal amount may leak past the adapter. There is **no minimum amount** (their own example initiates ৳1).

**⚠️ The environment is chosen by the base URL alone, and credentials are not portable between them.** Sandbox credentials on the live host answer `1001 Invalid Credential.` on initiation — but on `/transaction-status` a wrong `merchantId` is **indistinguishable** from an unknown invoice. Never change `PAYSTATION_BASE_URL` without swapping the credentials with it.

### ✅ PayStation replaces SSLCommerz — 2026-09-10

SSLCommerz is gone: `SslCommerzClient`, `SslCommerzReturnController`, `SslCommerzWebhookController`, `SslCommerzClientTest` and `SslCommerzReturnTest` are deleted, its config block and `SSLCOMMERZ_*` env keys are removed, and `sslcommerz` is no longer in `PaymentGatewayResolver::SUPPORTED_GATEWAYS`. It is deliberately **not** mapped to `FakeGateway` as a fallback — a leftover row must fail to resolve rather than quietly resolve to a stand-in that would happily report a real transaction as settled. Both read paths that walk historical rows (`ReconcilePayments`, `payments:stuck`) already catch a resolver failure and log it, so a legacy row is skipped with a warning rather than crashing the nightly sweep. `RefundPayment` would throw on one, which is the right answer: nothing here can refund through a gateway this system no longer integrates.

**Verified against the live sandbox while it was written**, not after — the whole point of the exercise, given that SSLCommerz's first live call (2026-08-14) found a signature bug that had made every genuine IPN fail. Confirmed live: a session created through the real adapter returned a `https://sandbox.paystation.com.bd/checkout/…` URL that serves HTTP 200, and `/transaction-status` answered `processing` with `request_amount: "2500.00"` for it. Every fixture in `PayStationClientTest` is a body copied from a real call, not one inferred from the docs.

**Six things the live calls established that the published documentation does not say — four of which would have been money bugs if guessed at:**

1. **`trx_status` on a settled payment is `"successful"`, not `"success"`.** Their documentation lists the values as `processing | success | failed | refund`, and the IPN section says `"Success"`; the sandbox returns **`successful`**. Found only by completing a real payment (2026-09-10, MASTERCARD, ৳2,500, `trx_id c5ab1d6b-…`) — every static reading of the docs, and every test written from them, said `success`. `PayStationClient::TRX_SUCCEEDED` now accepts both, and each spelling has its own test so neither can be tidied away to match the docs. **What made this recoverable rather than a disaster is the `default => PENDING` arm**: an unrecognised status word left the payment `initiated` and the page spinning, so the money was still where it should be and one re-verify settled it. Had the default been "failed", a paid registration would have been cancelled and its seat released. That arm is now load-bearing, not defensive, and is documented as such.
2. **`2001 "Transaction not found in system"` is also what a wrong `merchantId` returns.** A valid invoice queried with a bogus merchant id is answered byte-for-byte identically to an invoice that does not exist. So `2001` can never be read as "this payment failed": one mistyped credential would otherwise fail *every* pending payment in the system and release all their reserved capacity. `PayStationClient::verify()` maps it to **pending**, and `test_a_not_found_answer_is_pending_because_it_is_also_what_bad_credentials_return` pins it. This is the same shape as REVE's `Text: REJECTD` (2026-08-22), and the third time a vendor's error word has turned out to be ambiguous.
3. **`status_code` is a string** (`"200"`, `"1008"`), never an integer, and **every failure is HTTP 200** with the real outcome in the body. Nothing may key off the HTTP status.
4. **`/initiate-payment` answers `Content-Type: text/html`** while returning a JSON body. `Response::json()` decodes on the body alone so this works, but nothing here may ever branch on content type. (`/transaction-status` does send `application/json`.)
5. **`1001 Invalid Credential.` exists** and is absent from their published error table, which lists only 200, 1008 and 2001.
6. **`/v2/transaction-status` returns a 500 HTML error page** for a `trxId` it does not know, rather than an error envelope — which is why v2 is deliberately unused. It is keyed by `trxId`, which only exists *after* a payment completes, so it cannot answer "did this settle?" for the pending payments that need asking; and its error shape would surface as an exception on the ordinary not-paid-yet path. **v1 by `invoice_number` is the sole verification path**, and it turns out to return `request_amount` too — a field the published v1 list omits.

**`gateway_reference` is our own `payment_number`, sent as PayStation's `invoice_number`.** It is the only key that works on every path: we hold it before the payer reaches the gateway, the status API accepts it, and the IPN carries it back. **An invoice number is single-use** — a second `/initiate-payment` with the same one answers `1008 Duplicate invoice number.` — so a payment row can only ever be initiated once. `PayStationClient` translates 1008 into that sentence rather than passing the vendor's three words through, because "Duplicate invoice number" tells an operator nothing about what to do.

**The IPN has no signature, and that needed a third state rather than a lie.** PayStation's docs say `Auth: None` outright: the IPN is a plain unauthenticated JSON POST. Reporting `signatureValid = false` would claim a check had failed; reporting `true` would be false. `GatewayWebhookResult` now carries `authenticity` — `verified` / `invalid` / `unsigned` — derived from `signatureValid` when a caller doesn't pass it, so every existing signed-gateway call site is untouched.

- `ProcessGatewayWebhook` gates on `mayTriggerVerification()`, which refuses only `invalid`. An `unsigned` notification is allowed through **because of how narrow acting on it is**: it never settles anything, it only prompts a fresh server-to-server `verify()` that re-reads status *and* amount over a channel the caller cannot influence. A forged IPN naming a real invoice therefore buys exactly one outbound status lookup — which is why the route is `throttle:300,1`.
- `payment_transactions.signature_valid` stores **NULL** for an unsigned notification via `recordedSignatureValid()`, and the row's `status` is `unsigned`. "We checked and it failed" and "there was nothing to check" must not look the same in an audit trail.
- The body's own `trx_amount` is **ignored**. PayStation's integration checklist tells merchants to compare it against the expected amount, but a value the caller supplies cannot check itself; the comparison is made against the gateway's own answer instead, which is strictly stronger. `test_an_inflated_amount_in_the_body_is_ignored` proves a forged amount reaches `amount_mismatch`, not `succeeded`.
- `PAYSTATION_IPN_IP_ALLOWLIST` matters more here than it did for a signed gateway — it is the only network-layer check there could be — but it is still left empty rather than guessed, because a wrong range silently drops real payment notifications. **The IPN URL is configured per merchant in PayStation's dashboard**, not sent with each payment, so it must be registered with them before any notification arrives at all.

**One callback URL, so the return leg lost its `next` parameter — and is safer for it.** SSLCommerz took three URLs (`success`/`fail`/`cancel`) and carried the frontend origin in a query param that had to be host-checked. PayStation takes one, so `PayStationReturnController` puts the **payment ULID in the path** and derives the redirect target entirely from `FRONTEND_URL`: there is no caller-supplied redirect target to abuse, and nothing depends on the gateway preserving our query string. Because one URL serves every outcome, the `payment_status` hint is derived from whether a `trx_id` came back — untrusted input doing a deliberately narrow job, since the public site treats both `success` and `fail` as "poll `/payment/verify`" and only the banner it eventually shows comes from server state. `cancel` is never emitted: a payer who abandoned may still have a session that settles moments later, and not polling is the one outcome that leaves a paid registration looking unpaid.

**Refunds are an explicit out-of-band acknowledgement, because PayStation has no refund API.** `refund` appears only as a value of `trx_status` — something that can be reported, not caused — so the money moves in their merchant panel and nowhere else. `PaymentGatewayInterface` gained `supportsGatewayRefund()`; when it is false, `RefundPayment` demands both `acknowledged_out_of_band` and the panel's own `gateway_refund_reference`, and throws `OutOfBandRefundRequiredException` (422, code `out_of_band_refund_acknowledgement_required`) without either. Recording a refund here voids the ticket and releases the seat, so booking one that never happened would leave an attendee with no ticket *and* no money back. The resulting `payment_transactions` row is `status = acknowledged_out_of_band`, never `success` — no request was made to a gateway, and a row claiming one would read as gateway evidence during a dispute. The admin dialog asks for both up front (`GATEWAYS_WITHOUT_REFUND_API` in `resources/js/features/finance/types.ts`); the server is the authority either way.

**Two real pre-existing bugs this uncovered in `RefundPayment`, both found by the new tests rather than by review:**

1. **No refund had ever recorded who approved it.** `gateway_refund_id`, `approved_by_user_id`, `approved_at` and `processed_at` are all outside `Refund::$fillable`, so `Refund::create()` silently dropped every one of them — the exact trap this file already documents for `qr_codes.image_media_id`. They stay non-fillable (an authority column must not be settable from any array that happens to carry the key) and are written with `forceFill()` before a single `save()` instead.
2. **A refund could die with a SQL error mid-transaction.** `UPDATE ticket_types SET quantity_sold = quantity_sold - 1` on a `BIGINT UNSIGNED` column raises SQLSTATE 22003 when the value is zero. It is now guarded in the `WHERE`, not wrapped in `GREATEST` — MySQL evaluates the unsigned subtraction, and errors, before any surrounding function sees it.

**Still open:**

- ~~No completed sandbox transaction.~~ **Closed the same day** — a real card payment was completed through the hosted checkout, and it immediately found the `successful` spelling above. Full chain proven end to end on that one transaction: payment `succeeded` at 250000 paisa with `trx_id c5ab1d6b-ffb0-43c8-b7a8-3fc075b55503`, registration `REG-NRPC2AMC` → `confirmed`, ticket `DEC100-CEN-2005-00001` issued by the queued job with its QR row active and both the QR image and the PDF rendered as private media. What is *still* unexercised is the failure path: no declined or cancelled sandbox transaction has been observed, so the words in `TRX_FAILED` are the documented ones plus plausible variants, and an unrecognised one stays pending (which the expiry sweeper then handles). Worth one deliberate declined payment before launch.
- **`payer_mobile_no` is not always a mobile number.** On a card payment it carries the masked PAN (`512345xxxxxx0008`). Nothing reads it into `payments.payer_msisdn` today, but do not start: a field labelled as a phone number that sometimes holds a truncated card number will be rendered as a phone number by whoever wires it up next.
- **`pay_with_charge` is 0** (the merchant absorbs PayStation's fee). At 1 the payer is billed our amount *plus* the fee, so the money that moves is larger than `amount_due_paisa`. `verify()` compares against `request_amount` rather than `payment_amount` precisely so that stays workable, but every figure shown before checkout and every reconciliation total is still computed from our amount. Decide it deliberately.
- **The IPN URL must be registered in PayStation's dashboard** (`{APP_URL}/webhooks/paystation`). Until it is, no notification ever arrives and the return page's `/payment/verify` poll is the only settlement path — which does work, and is why the poll exists.
- **`missing_locally` reconciliation** and the direct bKash/Nagad/Rocket adapters are unchanged and still open, but neither is now on the critical path.

### 🗄️ SSLCommerz sandbox verified live, and two real defects fixed — 2026-08-14 — **SUPERSEDED 2026-09-10**
...
### ✅ Issuance emails a registration confirmation; the QR ticket is sent by staff — 2026-09-17

A ticket used to be emailed the moment it was issued — QR, number, gate details and the share card in one message. That is now **two messages**, and only the first is automatic:

| | template | when | carries | channels |
|---|---|---|---|---|
| registration confirmed | `registration_confirmed` | automatically, on `TicketIssued` | the "আমি থাকছি!" share card, a link to the registration | email |
| the ticket | `ticket_delivered` | **when staff send it** from the admin console | QR, ticket number, gate details, notes | email, SMS |

The point is control over when QR codes exist in inboxes: the organisers can hold every ticket back until the event is close, while the buyer still hears immediately that their money bought a seat — and the email they forward to friends has nothing in it that admits anyone.

- **Nothing sends `ticket_delivered` automatically any more.** `QueueTicketDeliveredNotification` became `QueueRegistrationConfirmedNotification` (`CHANNELS = ['email']`), and `ticket_delivered` moved onto `ResendTicketNotification::TEMPLATE_KEY`, because that action — behind `POST /admin/tickets/{ticket}/resend`, `resend-all` and `resend-preview` — is now the **only** path that sends the ticket. The routes, class and job keep the `resend` name (the mechanism is unchanged and it still serves the desk request), but every operator-facing label is now *Send ticket* / *Send all tickets* / *Send to selected*. **There is no new endpoint and no new permission**: the buttons that existed are the send.
- **`ProvidesMailPresentation::mailPresentation()` now takes the template key.** One notifiable (the ticket) is behind two emails, and the outbox row's `template_key` is the only thing that says which. `Ticket::mailPresentation()` routes: `ticket_delivered` → `TicketMailPresentation::for()` (QR, facts, notes — **no share card any more**), `registration_confirmed` → `registrationConfirmed()` (share card, CTA — no `facts`, no `ticketId`, no `qrPng`, no `notes`, so the shell renders neither the ticket card nor the gate strip), anything else → the plain shell. `test_the_registration_confirmed_email_carries_nothing_that_admits` asserts the absence of the QR part, the ticket number and the gate strings; its converse asserts the ticket email has the QR and not the card.
- **The share card is best-effort in the registration email, exactly as it was in the ticket email.** A render failure logs a warning and the email goes out with the body copy and the link; a voided ticket's queued email gets no card.
- **One payload for both messages.** `TicketNotificationPayload` gained `registration_number`; both templates' `variables` list it, so an editor can move a placeholder between the two. The new template deliberately does **not** interpolate `{{event_name}}`: that variable is the *English* name (it exists for the SMS) and is `''` where neither `event.name_en` nor `event.name` is set — which the dev database proved by rendering "-এ আপনার নিবন্ধন" — while the masthead already names the event with an `app.name` fallback.
- **It is also the receipt — a completed payment sends exactly one email.** `payment_succeeded` and `payment_manual_verified` are now **WhatsApp-only** (their email rows stay seeded but unsent): a settled payment queues issuance, issuance sends this email seconds later, and "Payment received" arriving right before "Registration confirmed" was two emails for one event. `TicketNotificationPayload::amount_bdt` (from `registrations.total_paisa` — Ticketing does not reach into Payment's rows, and the two agree by construction since a payment settles only against `amount_due_paisa`) lets the body say *"We have received your payment of BDT 1,520.00, and your registration REG-… is complete"*. The trade is real and deliberate: if issuance fails, the payer hears nothing until the job is replayed from `failed_jobs`. Counter cash sales were already on this path (`CollectCashPayment` dispatches no payment event).
- **The SMS moved with the ticket.** Booking, payment and now registration confirmations are email-only; the one SMS a purchase sends ("Your QR ticket has been emailed. Please present it at the gate.") goes out when staff send the ticket and tick SMS. So a purchase now costs **zero** SMS until the organisers decide to send tickets, and the cost is shown in the bulk dialog before anything is billed.
- **The `ticket_delivered` copy the client approved this morning is untouched** — "we are pleased to confirm that your ticket has been issued" still reads correctly for a ticket sent later. The new `registration_confirmed` bodies (EN/BN) say the seat is reserved and the QR ticket will follow in a separate email.
- `mail:test --ticket=X` sends the ticket email as before; `--registration` beside it sends that ticket's registration-confirmed email instead — the one issuance actually sends.
- 3 new tests in `TicketShareCardTest`, 1 new and 4 rewritten in `NotificationOutboxTest` (the receipt interpolation; issuance, payment-succeeded and manual-verified channel lists, each seeding the excluded channel *active* so they prove the listener chose, not that a template was missing), 1 rewritten in `AdminCashRegistrationTest`. Full suite **993 passing / 2 skipped**, Pint and PHPStan level 8 clean, SPA typecheck clean. OpenAPI regenerated (still 129 paths — summaries on the three send endpoints).

**Needs `php artisan db:seed --class=NotificationTemplateSeeder`** on any existing database — until it runs there is no `registration_confirmed` row, `QueueNotification` finds no template, and issuance writes **no outbox row at all**: the buyer gets nothing and the delivery log shows nothing. Run on this dev box already. The rendered registration email was checked at 700px from the real seeded template through the `array` transport (card as a 326 KB inline JPEG, no QR part, no ticket number in the HTML); the ticket email through the same path still carries `ticket-qr.png` and the number. Nothing was sent.
...
### ✅ The ticket card's words are CMS-editable, and page seeders have an additive mode — 2026-09-19

The `/tickets` price card's copy — badge, tagline, "for your seat", the button label, both list headings, "(optional)", and the *Every ticket includes* / *Bringing family?* lists — was hardcoded in `centennial-celebration` (`features/ticket-system/config.ts`, `lib/i18n/copy.ts`). The CMS `ticket_pricing` block carried only the heading above it, so an editor could change the prices (ticket type) and the heading but not one word on the card. Reported as "I can't update this, all content from the CMS".

- **The `ticket_pricing` block now carries the card's copy** (`blocks.ts`: `card_badge`, `card_tagline`, `card_price_caption`, `card_cta_label`, `includes_title`, `includes[]`, `family_title`, `family_optional_label`, `family_includes[]`), seeded verbatim by `TicketsPageSeeder`. **Every figure on the card stays live from the ticket type** — prices, the family limit, the free-child age — and has no field here; the block description says so. Blank field = the copy the card ships with; an empty list = the designed list, and the first row an editor adds replaces it whole. Verified against the running dev stack: probe values written to the block rendered on `/tickets`, and a two-row list replaced the five shipped items.
- **`php artisan content:backfill-seeded-fields TicketsPageSeeder`** — the reason this needed more than a schema change. A page seeder's normal re-run (`db:seed --class=…PageSeeder`) is `updateOrCreate` on *every* block, so on a live database it would seed over every edit an editor has made to that page — the FAQ answers, the hero, all of it. The additive command runs the same seeder with `ContentSeedMode::$fillMissingOnly` set: a stored block of the same type at the same position gains only the keys it lacks, a cleared-to-empty key stays cleared, a block of another type or an extra trailing block is left alone, and the page row itself (title, SEO, publication state) is untouched — every page seeder now creates its row through the trait's `seedPage()` so that guarantee covers the whole page. Four tests in `tests/Feature/Content/BackfillSeededContentFieldsTest.php`, one of which proves the ordinary re-seed still overwrites, so the additive mode is the only safe one on live.
- **The flag lives in its own class, not on the trait.** PHP gives each class using a trait its *own* copy of the trait's statics, so `SeedsContentBlocks::$flag = true` was invisible to `TicketsPageSeeder::$flag` — the first cut ran the seeder in full and the tests caught it.
- **⚠️ Live needs `php artisan content:backfill-seeded-fields TicketsPageSeeder` once**, after deploy. Until then the editor shows the nine new fields empty over a card still reading the shipped copy — correct, but confusing. Do **not** run `db:seed --class=TicketsPageSeeder` on live.
- **Use the additive command whenever a block type gains fields from now on** — this will recur, and it is the only way to surface new fields on an edited page without losing the edits.
- **The `pricing_rules` block got the same treatment the same day**, with one difference that matters: its three rule bodies *contain* the live figures, so the fields are written with placeholders — `{{base_price}}`, `{{student_price}}`, `{{member_price}}`, `{{max_members}}`, `{{free_age}}` — that `TicketPricingRules` fills from the ticket type at render, already localised (Bangla digits on the Bangla side). An unknown placeholder is left as typed, so a typo shows on the page rather than silently disappearing. The seeded strings are byte-for-byte the component's own fallbacks, verified by diffing the rendered section before and after the backfill. `{{student_price}}` is empty on a ticket with no student rate; the shipped fallback switches to a "flat, whoever you are" sentence in that case, an editor's own body does not — the field help says so. Same backfill command covers it.

### ✅ A blank CMS field renders nothing on the public site — 2026-09-19

Every section of `centennial-celebration` used to keep the copy the design shipped with as its fallback: a field an editor cleared came straight back as the shipped wording, with nothing in the CMS to explain where it came from. At the client's request ("if any field is empty don't show any dummy content — for all"), that rule is reversed site-wide (`centennial-celebration` commit `a144657`).

- **`fields()` in `features/content/blockFields.ts` now answers a blank field explicitly** — `EMPTY_TEXT` (`{en:"",bn:""}`), `""`, `[]` — instead of `undefined`, so a component's `= DEFAULT` prop default never fires from the CMS path. `useLocale()` gained `has()` (a type guard: text in the active language), `isBlank()` covers the server-side block mappers, and `f.any([...keys])` lets a mapper skip a whole block whose band it draws itself. Every section component hides whatever has nothing in it — a line, a button (label *and* URL required), a repeater row, a list, a card, or the whole section when it is all blank — and margins follow so nothing leaves a stray gap.
- **The shipped copy still renders on a page with no CMS page behind it at all.** The `Shipped…Page` fallbacks call the sections with no props, so a dev checkout with an empty database, or an API outage, still shows the designed page. It is only a *field* that no longer falls back. `yearPill` keeps the one distinction that needs it: `undefined` (no CMS) draws 1927–2027, `""` (cleared) draws no pill.
- **Artwork the layout cannot stand without keeps its file**: the homepage hero art and the site logo. A blank repeater image (guest photo, attraction tile, album cover) draws the tile without a picture rather than crashing — `next/image` throws on an empty `src`, which is what an emptied image field used to hand it.
- **The admin `blocks.ts` help texts that promised a fallback were rewritten** ("Leave empty for the shipped six" → "An empty list draws no grid", etc.). The contact hero's channel values still fall back to the `contact.*` event settings — that is real contact data, not shipped copy — and the FAQ and sponsor grids keep their empty-state lines because they list live admin resources, not CMS fields.
- **Verified on the dev stack, not only by types**: with the old code stashed and then restored, every CMS-driven page's visible text was snapshotted and diffed — identical apart from fields that really are blank in the dev database; then the event hero's body and one fact, the whole glance rail, the venue notes and map label, and the CTA's eyebrow and second button were blanked in the database and exactly those disappeared, everything else staying put. `tsc`, ESLint (0 errors) and `next build` clean.

**⚠️ Deploy order matters now, and the live site will change.** Anything blank on live is about to *disappear* rather than show the shipped text — on the dev database that was the tickets hero breadcrumb and the year pill on the Alumni and Contact heroes. And the ticket card's copy and the three pricing rules live in fields the live Tickets page does not yet hold, so **deploy the backend and run `php artisan content:backfill-seeded-fields TicketsPageSeeder` on the host *before* deploying the public site** — the other way round leaves a price card with no words and no rules until the command runs. After the frontend deploys, walk every page and fill in anything that went missing; it was shipped copy the CMS never held.
...
A single Laravel application at the repo root, which serves **both** the API and the admin dashboard SPA. **`.github/workflows/backend-ci.yml` is the one that actually deploys** — lint, static analysis and `composer audit` on every push, the test suite on pull requests, then a `deploy` job that rsyncs the release to Hostinger over SSH and runs migrations, `qr-signing:generate-key --if-missing`, `optimize:clear` + the three caches, and `admin:create-super-admin --if-missing` on the host (see [docs/09](docs/09-hostinger-deployment.md), and [§First Super Admin](#commands) for the bootstrap). That rsync uses `--delete`, so **anything living only on the server is removed unless the workflow names it** — `.env`, `storage/` and `public/storage` are excluded, and `.htaccess`, `.well-known/` and `.user.ini` are `protect`ed from deletion while still letting the repo's own `public/.htaccess` deploy.

**The Laravel tree lives outside the web root, as of 2026-09-04.** `HOSTINGER_DEPLOY_PATH` is `.../<domain>/laravel` and a second secret (or repo variable) `HOSTINGER_PUBLIC_PATH` is `.../<domain>/public_html`; a final "Publish Laravel public directory" step rsyncs `laravel/public/` into `public_html/` **on the host**, so nothing is uploaded twice and no part of the framework ever lands in a directory Apache serves. It replaces the rewrite-into-`public/` `.htaccess` trick, which protected `.env`, `vendor/`, `app/` and `artisan` only for as long as that one file survived — and it kept not surviving, at which point all four answered 200. The step is **opt-in**: with `HOSTINGER_PUBLIC_PATH` unset it skips and the two older shapes (document root moved to `.../public`, or `public_html` symlinked at it) deploy exactly as before. It refuses outright — before the `--delete` — when the publish target is `/` or `$HOME`, or contains the Laravel tree itself, which is what the old "deploy path is the document root" layout looks like. `admin:create-super-admin` no longer passes `--generate-password`: it printed an invented password into a build log anyone with repo access can read, so the credential now only comes from the host's own `SUPER_ADMIN_EMAIL`/`SUPER_ADMIN_PASSWORD`, and the step skips cleanly when neither is set. `.github/workflows/deploy.yml` (Phase 9) is the **Docker/GHCR path and is not wired to a real host** — it builds and publishes the production image and carries staging/production deploy jobs that are no-ops until `STAGING_HOST`/`PRODUCTION_HOST` are set.
...
**Forgotten password (staff)** — `POST /admin/auth/forgot-password` and `POST /admin/auth/reset-password`, with `/forgot-password` and `/reset-password` pages in the SPA. It did not exist before 2026-08-31: the `password_reset_tokens` table and the `config/auth.php` broker were Laravel defaults nothing had ever used. Built on the framework's broker rather than hand-rolled, so token hashing, the 60-minute expiry and single use come from code that is already trusted.

- **Neither endpoint ever confirms an account exists.** `forgot-password` returns one body always; there is deliberately no `exists:users,email` rule, because a validation error naming an unknown address is the enumeration this is avoiding. Suspended and soft-deleted accounts are skipped silently for the same reason. `reset-password` answers an expired token, a wrong token and an unknown address with the same message on the same field.
- **A mailer failure must not change the answer either.** `Mail::send` throwing would 500 for an address that has an account and 200 for one that does not — a misconfigured mailer turning the most carefully-written endpoint here into an oracle. It is caught and logged; the caller sees the same 200. There is a test.
- **`throttle:staff-password-reset`** — 3/hour per address, 20/hour per IP. Inbox flooding and address enumeration first, cost second. `reset-password` stays on the shared `api` bucket: its token is 64 random characters, so guessing is not the threat.
- **A reset does not sign you in and does not bypass 2FA.** No token is issued; the next login goes through TOTP as usual. Reading a mailbox is not the second factor.
- **It revokes every session** (not "every other" — nobody proved which one is theirs) and clears the lockout, via `ChangeStaffPassword::afterReset()`, audited as `password_reset` rather than `password_changed` because how a credential moved is the point of the row.
- **`StaffPasswordResetMail` is deliberately outside the notification outbox**, unlike every other email here. The outbox honours a per-channel kill switch (an operator silencing email would silence account recovery), renders an editable template (a mis-saved one must not be able to remove the link), and is drained by a queue worker docs/09 §0 says cannot run on shared hosting. It is sent inline instead. The URL is built from `config('app.url')`, never the request — a caller-supplied origin would mail a real staff member a real token pointing at a site they control. `User::sendPasswordResetNotification()` is overridden to do this.

⚠️ **Sending is not arriving.** `StaffPasswordResetMail` sends `multipart/alternative` — HTML plus a real plain-text part — because an HTML-only message is a long-standing spam signal and a reset link in a junk folder is a recovery path that does not exist. That is the only half of the problem code can fix; the rest is DNS (SPF, DKIM, DMARC) and `From`-domain alignment, and until those exist a reset landing in spam is the expected outcome rather than a fault. [docs/09](docs/09-hostinger-deployment.md) has the record set and how to verify it. **`NotificationMail` — the shell every attendee email uses, including the ticket confirmation — is still HTML-only**, which is the same defect at 12,000 times the volume and wants fixing before the ticket blast.

⚠️ **This depends on the host's `MAIL_MAILER` actually working.** If the live `.env` still has `log`, reset emails land in `storage/logs/laravel.log` and nobody receives them — and by design that failure is quiet at the endpoint, so check the log rather than the response.

**Also fixed: the lockout counter never reset when a lockout elapsed.** `registerFailedAttempt` incremented from whatever the count already was, and only a *successful* login cleared it — so once an account hit five failures, the first mistype after every 15-minute cooldown took it to six and re-locked immediately. That is not "five tries then a pause", it is one try per cooldown for ever, which is exactly what someone copying a generated password out of a CI log will hit. An elapsed lockout has been served, so the count now starts again.

**A login that answers "This password does not use the Bcrypt algorithm":** the *stored* value is not a hash the configured hasher can read — a row whose password was pasted into a database client, or imported from another system. `Hash::check()` throws rather than returning false in that case, and the check is on the column, not on what was typed, so every attempt against that account used to become an unhandled exception: a 500 on a public endpoint, and a stack trace wherever `APP_DEBUG` is on. **`App\Domain\Shared\Support\PasswordHash` is now the only way this codebase compares a password** — `matches()` answers false where `Hash::check()` would throw (a hash nothing can read cannot match anything, and "this account's hash is malformed" is not a fact to hand an unauthenticated caller), and `isUsable()` asks the real hasher whether a stored value could ever authenticate anyone. All four call sites — admin login, admin re-auth, attendee sign-in, attendee password change — go through it. An unreadable hash at admin login is deliberately **not** counted as a failed attempt: nobody guessed anything, and counting it would lock the account after five tries and answer 423, moving the message further still from the cause. It is logged as a warning naming the user and the fix instead. `admin:create-super-admin --if-missing` repairs such an account — password, role, status and lockout — which is the one exception to its "never touch an existing account" rule, because an account nobody can log into is not one whose settings deserve protecting.
...
**`--if-missing` is the deploy path**, run by `backend-ci.yml`'s deploy job (and by both jobs in the unwired `deploy.yml`) after `config:cache` (which is the point at which config reflects the `.env` the release is running against). It reads `SUPER_ADMIN_EMAIL`/`SUPER_ADMIN_PASSWORD`/`SUPER_ADMIN_NAME`/`SUPER_ADMIN_PHONE` from the host's own `.env` via `config/admin.php`, so no credential passes through the workflow file, the SSH command line, or `ps`. Three behaviours, each deliberate:

- **Nothing configured → skip, exit 0.** Both halves must be set for the deploy to create anything, so an environment that provisions its admin by hand never has one invented for it — and a deploy is never blocked by the absence. This is also what makes deleting `SUPER_ADMIN_PASSWORD` from `.env` after first use the intended end state rather than a loose end: it is a plaintext credential for exactly as long as it sits there, and every later deploy takes the "already exists" branch and never reads it again.
...
**`--generate-password` still exists as a flag, and the deploy no longer passes it (removed 2026-09-04).** It invents a 20-character password when none is configured, creates the account at `config('admin.super_admin.email')`, and **prints it once**, because only a bcrypt hash is stored and no code path can recover it afterwards. That made a deploy work with nothing configured at all — at the cost of putting a live credential into a build log that everyone with read access to this repository can open, and which outlives any memory of the account. It is fine to run by hand on a host you are watching; it is not fine in CI. The first account now comes from `SUPER_ADMIN_EMAIL`/`SUPER_ADMIN_PASSWORD` in the host's `.env`, or from running the command interactively (docs/09 §6).
...
- **Seed data belongs in versioned config/seeders, not ad hoc.** RBAC roles/permissions are seeded from `config/rbac.php` via `RbacSeeder` — add new permissions there, not directly through Spatie's API. `TicketTypeSeeder` follows the same pattern; extend it rather than creating rows manually in migrations.
- **A new event setting goes in `config/event_settings.php`, and nowhere else.** That catalogue — not the `event_settings` table — is what the Settings screen renders and what `PATCH /admin/settings/{key}` resolves against, so a key added there is on screen and savable the moment the release deploys; the row is created by the first save. `EventSettingSeeder` now just materialises the same catalogue, so forgetting to run it hides nothing. Metadata (group, type, label, description, visibility, `is_encrypted`) is code-owned and refreshed from config on read; the *value* is admin-owned and only ever written by a save or by seeding a row that does not exist yet. A `datetime` default may be relative (`'+6 months'`) — `castForStorage()` parses it at use time, which is why it must never be a `now()` call in that file: `config:cache` would freeze it at build time. Keep `label` ≤ 120 and `description` ≤ 255 characters; `SettingCatalogueTest` fails the build otherwise. **This replaced a real live-server failure**: the whole SMS gateway group was invisible on production because those keys were added to the seeder after that environment was last seeded, the deploy runs migrations and no seeders, and the update endpoint answered 404 for a key with no row — so the credentials could be neither seen nor entered.
